...
static void   dev8(Device *d, Uint8 a, Uint8 b) { d->dat[a & 0xf] = b; d->talk(d, a & 0x0f, 1); }
...
void   mempoke16(Uint8 *m, Uint16 a, Uint16 b) { poke8(m, a, b >> 8); poke8(m, a + 1, b); }
Uint16 mempeek16(Uint8 *m, Uint16 a) { return (peek8(m, a) << 8) + peek8(m, a + 1); }
static void   devpoke16(Device *d, Uint8 a, Uint16 b) { dev8(d, a, b >> 8); dev8(d, a + 1, b); }
...
/* Memory */
static void op_pek(Uxn *u) { Uint8 a = pop8(u->src); push8(u->src, peek8(u->ram.dat, a)); }
static void op_pok(Uxn *u) { Uint8 a = pop8(u->src); Uint8 b = pop8(u->src); poke8(u->ram.dat, a, b); }
...
static void op_deo(Uxn *u) { Uint8 a = pop8(u->src), b = pop8(u->src); dev8(&u->dev[a >> 4], a, b); }
/* Arithmetic */
